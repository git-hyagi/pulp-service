...
TASKS_ENDPOINT = "/api/pulp/admin/tasks/?fields=pulp_created&fields=started_at&"
...
def run():
    ...
    current_interval = query_date_time
    while current_interval <= datetime_now:
        end_date = current_interval + timedelta(seconds=bucket_size_in_seconds)
        url = base_addr + TASKS_ENDPOINT + "started_at__gte=" + current_interval.strftime(DATETIME_FORMAT) + "&finished_at__lte="+end_date.strftime(DATETIME_FORMAT)
        ...
        if response_json['count'] > 0:
            data[current_interval.strftime(DATETIME_FORMAT)]["running"] = response_json['count']
        

        url = base_addr + TASKS_ENDPOINT + "state=running"
        response = requests.get(url,auth=(username,password))
        ...
        if response_json['count']:
            for result in response_json["results"]:
                if current_interval < datetime.strptime(result['started_at'],DATETIME_FORMAT) < current_interval:
                    data[current_interval.strftime(DATETIME_FORMAT)]["running"] = response_json['count']

        current_interval = end_date
